...
const CustomFormComponent = memo(function CustomFormComponent({
  formIcon,
  formName,
  fields,
  initialData = {},
  ...
  // State untuk error per field (objek: { fieldName: [errors] })
  ...
  const handleChange = useCallback(
    (e) => {
      ...
      // Hapus error field tersebut saat user mulai mengetik lagi
      if (fieldErrors[name]) {
        setFieldErrors((prev) => {
    ...
    <div className="space-y-8">
      ...
      {(formIcon || formName) && (
        <div className="flex flex-col items-center">
          {formIcon && (
            <div
              className={`mb-4 flex h-14 w-14 items-center justify-center rounded-2xl bg-purple-600 text-white shadow-xl shadow-purple-200 dark:shadow-none`}
            >
              {formIcon}
            </div>
          )}
