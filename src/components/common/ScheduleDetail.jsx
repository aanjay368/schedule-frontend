        ...
        <div className="space-y-1">          
          <div className="flex items-center gap-2 text-sm text-purple-600 xl:font-medium dark:text-purple-400">
            ...
            <div className="w-63 overflow-hidden">
              <span className="px-70 marquee sm:animate-none sm:transform-none">{schedule.id}</span>
            </div>
          </div>
        </div>
      </div>
          ...
          items={[
            {
              icon: <User size={18} />,
              label: "Nama Staff",
              value: <Link to={`/staff/profile/${schedule.employee.id}`}>{schedule.employee.nickname}</Link>,
            },
            {
              icon: <Info size={18} />,
