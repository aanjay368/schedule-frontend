...
import {searchBackupHitoryService} from "../service/backupHistoryService";
import Card from "../../../components/ui/Card";
import {DetailItem} from "../../../components/layouts/DataDetailLayout";
import BackupFilter from "../components/forms/BackupFilter";
...
export default function BackupHistory() {
  ...
  const fetchSubmissions = useCallback(async () => {
    ...
    setError(null);
    try {
      const response = await searchBackupHitoryService(filters);
  ...
  const handleFilterSubmission = useCallback(() => {
    openModalOverlay(
      <BackupFilter filters={filters} onClose={closeModalOverlay} />,
      "Filter Ngefull",
      "Atur filter Ngefull untuk menyaring riwayat aktivitas Ngefull kamu.",
      "md",
    );
      ...
      <header className="flex flex-col gap-6 px-2 sm:flex-row sm:items-end sm:justify-between">
        <div className="space-y-1">
          ...
          <p className="text-sm text-slate-500">
            Pantau semua Aktivitas ngefull di sini.
          </p>
        </div>
        ...
        {isLoading ? (
          ...
          <div className="space-y-12">
            <p className="border-b border-slate-100 pb-6 text-sm text-slate-500 dark:border-slate-800">
              Terdapat {paging.totalElement} Aktivitas Ngefull
            </p>
 
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-3">
              {submissions.map((req) => (
                <Card key={req.id} className="relative space-y-4 px-2 py-4">
                  <DetailItem label={"Nama Rekan"} value={req.debtorName} link={`/staff/profile/${req.debtorId}`}/>
                  <DetailItem label={"Tanggal"} value={req.workDate}/>
                  <DetailItem label={"Shift"} value={req.shiftName}/>
                </Card>
              ))}
            </div>
